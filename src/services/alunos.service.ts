...
async function updateAluno(id: string, nomeCompleto: string, cpf: string, email: string, telefone: string, estado: string, municipio: string, rua: string, bairro: string, numeroDaCasa: number, dataDeNascimento: string, rg: string, dataExpedicaoRg: string, estadoDeNascimento: string, cidadeDeNascimeto: string): Promise<string> {
    try {
        ...
        let resposta = "";
        if (!nomeCompleto || !cpf || !email || !telefone || !estado || !municipio || !rua || !bairro || !numeroDaCasa || !dataDeNascimento || !rg || !estadoDeNascimento || !cidadeDeNascimeto || !dataExpedicaoRg) {
            resposta = 'Todos os campos são obrigatórios para cadastrar o aluno.';
            return resposta;
        }
        const response = await db.query(
...
export const alunoService = {
    createAluno: (id: string, nomeCompleto: string, email: string, telefone: string,   
        estado: string,
        municipio: string,
        rua: string,
        bairro: string,
        ...
        cidadeDeNascimeto: string) => createAluno(id, nomeCompleto, email, telefone, estado, municipio, rua, bairro, numeroDaCasa, dataDeNascimento, cpf, rg, dataExpedicaoRg, estadoDeNascimento, cidadeDeNascimeto),
